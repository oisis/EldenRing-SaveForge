...
| Zmiana kodu produkcyjnego | Phase 0..6 + Phase 6b + Phase 7a + Phase 9 dostarczone; późniejsze fazy pozostają wyłącznie design. Szczegóły w §17 i §17a. |
...
- **Decyzja o lokalizacji UI**: kontrolki żyją wewnątrz istniejącego dropdownu Templates w `SortOrderTab.tsx`, więc ta faza ship bez dotykania żadnego z czterech globalnych modali Templates. Globalny Templates shell nie udostępnia override, ponieważ jego ścieżki Apply v2 nie sięgały do writera v1 `inventory.workspace`; Phase 7a dostarczył ścieżkę Apply v2 `inventory.workspace`, ale nadal hard-coduje override jako `nil`, więc gdy ship odpowiednik v2 Phase 6b (Phase 7a.2), panel można zrelokować. Puste pola number oznaczają "zostaw tę klasę w spokoju" (odpowiadający pointer zostaje nil w payloadzie); inline element `weapon-override-error` blokuje Apply gdy panel jest enabled z obydwoma pustymi polami albo z dowolnym polem poza zakresem.
...
- **Out of scope**: weapon level override na ścieżce Apply v2 `inventory.workspace` (odroczone do Phase 7a.2 — ścieżka v2, która shipowała w Phase 7a, przekazuje hard-coded `nil` override); writery equipped-weapon (Phase 7b); zmiana `Infusion` lub `AoW` (wartości szablonu są passowane bez zmian); override per-broń (zamiast per-klasa); direct save mutation przez `core.PatchWeaponItemID` ze ścieżki template apply.
...
- **Out of scope (nadal future work)**: Phase 7a.2 — podłączenie Phase 6b weapon level override do ścieżki Apply v2 `inventory.workspace` (dzisiaj ścieżka v2 przekazuje hard-coded `nil` override). Phase 7b — equipment slot writer (zob. niżej). Phase 7c — equipped talismans writer. Phase 7d — spell loadout writer. Phase 8 — appearance via preset. Phase 10 — multi-character pack.
- **Wymaga osobnej decyzji użytkownika przed kontynuacją**: zakończone.
...
Praca Phase 7a.2 / 7b+ pozostaje wyłącznie design w tym dokumencie. Każda faza wymaga osobnej akceptacji użytkownika przed implementacją zgodnie z workflow z `~/.claude/CLAUDE.md`.
