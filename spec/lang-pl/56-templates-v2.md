...
| Zmiana kodu produkcyjnego | Phase 0..6 + Phase 6b + Phase 9 dostarczone; późniejsze fazy pozostają wyłącznie design. Szczegóły w §17 i §17a. |
...
- **Decyzja o lokalizacji UI**: kontrolki żyją wewnątrz istniejącego dropdownu Templates w `SortOrderTab.tsx`, więc ta faza ship bez dotykania żadnego z czterech globalnych modali Templates. Globalny Templates shell nie udostępnia override, ponieważ jego ścieżki Apply v2 nie sięgają do writera v1 `inventory.workspace`; gdy ship odpowiednik v2 Phase 6b (Phase 6b/v2 lub Phase 7+), panel można zrelokować. Puste pola number oznaczają "zostaw tę klasę w spokoju" (odpowiadający pointer zostaje nil w payloadzie); inline element `weapon-override-error` blokuje Apply gdy panel jest enabled z obydwoma pustymi polami albo z dowolnym polem poza zakresem.
...
- **Out of scope**: ścieżka apply v2 inventory / weapons (nadal blokowana przez scope guard `inventory.workspace` w `app_templates_v2_apply.go`); writery equipped-weapon (Phase 7a); zmiana `Infusion` lub `AoW` (wartości szablonu są passowane bez zmian); override per-broń (zamiast per-klasa); direct save mutation przez `core.PatchWeaponItemID` ze ścieżki template apply.
...
- Odpowiednik v2 Phase 6b weapon level override (apply-time override na przyszłym writerze v2 inventory / weapons) — gated przez Phase 7+, obecnie blokowany przez scope guard v2 `inventory.workspace` w `app_templates_v2_apply.go`.
...
Praca Phase 7+ pozostaje wyłącznie design w tym dokumencie. Każda faza wymaga osobnej akceptacji użytkownika przed implementacją zgodnie z workflow z `~/.claude/CLAUDE.md`.
