...
| 56 | [Templates v2 — Partially Implemented Extension](56-templates-v2.md) | `partial` | Addytywne rozszerzenie [55](55-build-template.md). Phase 0..6 + Phase 6b + Phase 7a + Phase 7a.2 + Phase 7b.0 + Phase 7b.1 + Phase 7c + Phase 9 dostarczone: addytywny schemat v2 (`version: 2`, top-level `selection`, `sections.profile` + `sections.stats`), globalny library shell Templates, publiczny YAML import/export (biblioteka na dysku pozostaje JSON), flow create-from-character z per-field selection profile/stats + preview + Save to Library, badge v2 w bibliotece i preview, **Phase 5 = Apply z biblioteki dla profile/stats przez `ApplyBuildTemplateV2FromLibraryToCharacter` oraz Phase 5D.2 direct imported-YAML Apply przez `ApplyBuildTemplateV2ToCharacterJSON` na canonical JSON wyprodukowanym przez preview importu (oba `mode: append`, `profile.class` pomijane, snapshot + rollback), Phase 6 = apply-time overrides dla tego samego subsetu profile/stats na obu powierzchniach przez frontend-only mutację canonical JSON przekazaną do tego samego JSON-owego endpointu, bez zmian backendu / bindings / `App.tsx`, Phase 6b = apply-time weapon level override dla istniejącej ścieżki Apply v1 `inventory.workspace` przez addytywną runtime-opcję `WeaponLevelOverride { Enabled bool, StandardLevel *int, SomberLevel *int }` na `app_templates.go::ApplyTemplateOptions`, aplikowaną po każdym template-side patchu broni przez `editor.UpdateWeapon` + `editor.ClampUpgrade(req, MaxUpgrade)`, surfacującą warningi `weapon_level_clamped` / `weapon_unupgradeable` na `report.Warnings`; kontrolki UI żyją wewnątrz istniejącego dropdownu Templates w `SortOrderTab.tsx`, `TemplatesShellModal` / `ApplyOverridesPanel` / `ImportTemplatePreviewModal` / `TemplateLibraryModal` bez zmian; bez zmiany schematu v2, bez writera equipment, bez direct save mutation, Phase 7a = pierwsza realna ścieżka Apply v2 dla `inventory.workspace` przerzucona przez aktywną `InventoryEditSession` / `InventoryWorkspaceSnapshot` przez nowy lookup `App.GetActiveInventoryEditSessionForCharacter(charIdx) → { active, sessionID }` oraz addytywne `ApplyTemplateV2Options.SessionID`; brak sesji → `IssueCodeInventorySessionRequired`, nieznana/niewłaściwa sesja → `IssueCodeInventorySessionInvalid`; mixed apply profile+stats+inventory.workspace jest atomowy przez dual slot+workspace snapshot rollback; Apply v2 inventory nigdy nie dotyka `slot.Data` bezpośrednio — `SaveInventoryWorkspaceChanges` pozostaje jedynym punktem commitu; `ImportTemplatePreviewModal` / `TemplateLibraryModal` podnoszą swoje zestawy supported-sections do `{profile, stats, inventory.workspace}`; `TemplatesShellModal` wyszukuje aktywną sesję dla każdej powierzchni Apply v2 (library, direct YAML import, URL import, `Apply with overrides…`), Phase 7a.2 = apply-time weapon level override podłączony do ścieżki Apply v2 `inventory.workspace` z Phase 7a przez addytywne pole `WeaponLevelOverride` na `ApplyTemplateV2Options` (reuse v1 typu i walidatora `validateWeaponLevelOverride` 1:1, podmiana dwóch hard-coded pinów `nil` w wywołaniach `applyTemplateItemsToWorkspace` dla inventory + storage na `opts.WeaponLevelOverride`); opcja runtime-only (nigdy w canonical JSON); nowy `WeaponLevelOverridePanel` osadzony w istniejącym `ApplyOverridesModal` i renderowany tylko gdy obecne `selection.inventory.workspace` (testids `apply-overrides-weapon-{panel,enabled,standard,somber,error}`); fast library Apply (`ApplyBuildTemplateV2FromLibraryToCharacter`) nadal nie wysyła override; strukturalnie poprawny override + template profile/stats-only jest silently ignored (brak items → no-op); warningi `weapon_level_clamped` / `weapon_unupgradeable` lecą do `ApplyTemplateV2Result.Preview.Warnings`; v1 dropdown Phase 6b w `SortOrderTab.tsx` bez zmian; bez nowej sekcji schemy v2, bez writera equipment, bez direct save mutation, bez zmian `App.tsx`, Phase 9 = import szablonów z `https://` URL przez `PreviewBuildTemplateImportYAMLFromURL` pod pełną listą guardów §12.3 (HTTPS-only, pre-connect IP filter dla literałów i DNS-resolved adresów, redirect re-check ×3, body cap 1 MiB, timeouty 10 s total / 5 s idle, strict TLS, Content-Type allowlist, brak auth/cookies/headers, struct-typed YAML decode reużyty bez zmian; preview URL reużywa tego samego `ImportTemplatePreviewModal`, więc Save to Library / Apply to character / Apply with overrides ship bez zmian na powierzchni URL; bez zmiany schemy biblioteki — metadata `sourceURL` nie jest dodawana)**. Apply v2 dla sekcji poza `{profile, stats, inventory.workspace}` pozostaje zablokowane; `ApplyBuildTemplateV2FromFileToCharacter` istnieje backend/bindings-side, ale celowo pozostaje niepodpięty w UI, bo ścieżka JSON jest WYSIWYG z preview. Phase 7b.0 (dostarczone 2026-05-31) = backendowa fundacja `(s *core.SaveSlot).WriteEquipment([]EquipmentWrite) error` dla slotów ChrAsmEquipment 0–9 + 12–15 (weapons + ammo + armor); strict GaMap-present-only resolution, validate-then-mutate atomicity, targeted hash 7 / 8 recompute, bez zmiany schemy szablonu / metody App / Wails binding / frontendu. Phase 7b.1 (dostarczone 2026-06-01) = end-to-end Apply v2 `sections.equipment` przez writer Phase 7b.0 — 14-slot `EquipmentSection` (weapons LH/RH 1/2/3, arrows1/2, bolts1/2, armor head/chest/arms/legs) z `EquipmentItemRef` niosącym `BaseItemID` + opcjonalne `Name` / `Upgrade` / `InfusionName` / `AoWItemID` (BaseItemID 0 = explicit clear, upgrade nil = match any); scanner w warstwie App + resolver przeszukuje **tylko** `slot.Inventory.CommonItems` (storage nieprzeszukiwane), emituje warningi `equipment_item_not_in_inventory` + skip na miss i `equipment_item_ambiguous` + first-match-wins na duplikaty; `WriteEquipment` dispatchowany wewnątrz istniejącego scope dual-snapshot rollback po profile/stats `SyncPlayerToData`; combo `equipment + inventory.workspace` hard-rejected (świeżość GaMap) z `equipment_inventory_combo_unsupported`; szablony equipment-only NIE wymagają sesji i reużywają kanoniczną ścieżkę JSON na powierzchniach library / direct YAML / URL; `ImportTemplatePreviewModal` + `TemplateLibraryModal` wystawiają equipment jako czwartą obsługiwaną sekcję; bez zmian `App.tsx` / `SortOrderTab.tsx` / `ApplyOverridesPanel.tsx`, bez panelu equipment override. Phase 7c (dostarczone 2026-06-01) = Apply v2 talismanów przez rozszerzenie `sections.equipment` o `talisman1..5` (indeksy ChrAsmEquipment 17–21, hash 8) zamiast wprowadzania oddzielnej sekcji `sections.equippedTalismans` — talismany żyją w tej samej strukturze `ChrAsmEquipment` i reużywają resolvera Phase 7b.1, combo guarda, wiersza preview, gatingu frontendu i rollbacku; `backend/core/equipment_writer.go` dodaje `EquipSlotTalisman1..5`, czwartą klasę `slotClassTalisman` (akceptuje tylko prefix handle'a `ItemTypeAccessory` 0xA0), rozszerzenie hasha 8 do 17–21 oraz enkodowanie `GaMap[handle]` bezpośrednio (bez maski `| 0x80000000`); resolver bramkuje non-empty refy talismanów względem aktywnej pojemności pouch'a `1 + effective profile.talismanSlots` (clamp do `MaxProfileTalismanSlots = 3`, cap = 4) z nowym warningiem `talisman_slot_pouch_insufficient` + skip; Talisman5 z `baseItemID > 0` zawsze warn + skip (vanilla cap = 4); `talisman5: {baseItemID: 0}` clear zawsze dozwolony; mixed `profile.talismanSlots + equipment.talismanN` podnosi cap zanim equipment apply się uruchomi; hard reject combo `equipment + inventory.workspace` bez zmian; brak zmiany source-code frontendu (`V2_APPLY_SUPPORTED_SECTIONS` już zawiera `equipment`); `frontend/wailsjs/go/models.ts` pokazuje brak diff'a (rozszerzenia talismanów to pola JSON-payloadu na `EquipmentSection`). Phase 7d.0 → 7d.4 (dostarczone 2026-06-02) = end-to-end import / preview / apply spells przez nową sekcję schematu `sections.spells` (14 nazwanych pointerów slot `Spell1..Spell14`, prefix `0x40000000` współdzielony przez sorceries ORAZ incantations, `BaseItemID == 0` = explicit clear, nil pointer = leave unchanged), batch writer `(s *core.SaveSlot).WriteSpells` z targeted recompute hash[10] (globalny `RecalculateSlotHash` pozostaje niepodłączony do produkcji; `WriteEquipment` nadal jest właścicielem hash[7] / hash[8]), helper `db.ItemIDToMagicParamID(itemID) = itemID & 0x0FFFFFFF` z maską 28-bit, pełny gating frontend + wiersz preview + badge biblioteki; spells współistnieją swobodnie ze wszystkimi pozostałymi shippowanymi sekcjami (brak restrykcji combo); wywołanie `WriteSpells` znajduje się **po** `vm.MapViewModelToSlot` ORAZ **po** `slot.WriteEquipment`, więc VM flush nie może clobberować nowego regionu spells. Phase 7d.4b (wiring `BuildTemplateV2ExportOptions.EquippedSpellsRaw` + checkbox Spells w `CreateTemplateV2Modal`), Phase 8 (appearance przez preset), Phase 10 (multi-character pack) nadal planowane. |
