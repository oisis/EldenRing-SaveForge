...
| Production code change | Phase 0..6 + Phase 6b + Phase 7a + Phase 9 shipped; later phases remain design-only. Detail in §17 and §17a. |
...
- **UI placement decision**: controls live inside the existing `SortOrderTab.tsx` Templates dropdown so this phase ships without touching any of the four global Templates modals. The global Templates shell does not expose the override because its v2 Apply paths do not reach the v1 `inventory.workspace` writer; Phase 7a shipped the v2 `inventory.workspace` apply path but still hard-codes the override to `nil`, so once Phase 6b's v2 counterpart ships (Phase 7a.2), the panel can be relocated. Empty number fields mean "leave that class alone" (the corresponding pointer stays nil in the payload); the inline `weapon-override-error` element blocks Apply when the panel is enabled with both fields empty or either field out of range.
...
- **Out of scope (still future work)**: Phase 7a.2 — wire Phase 6b weapon level override into the v2 `inventory.workspace` Apply path (today the v2 path passes a hard-coded `nil` override). Phase 7b — equipment slot writer (see below). Phase 7c — equipped talismans writer. Phase 7d — spell loadout writer. Phase 8 — appearance via preset. Phase 10 — multi-character pack.
- **Requires separate user decision before continuing**: completed.

### Phase 7b — equipment slot writer (new write path)

- **Goal**: implement a new public write path for `ChrAsmEquipment` slots 0..9, 12–15 (weapons + armor), respecting the encoded item-ID form and the hash 7/8 dependency documented in [06-equipment](06-equipment.md). Apply `sections.equipment` from the template through this new writer. Phase 7b is the second leg of Phase 7+ — Phase 7a already shipped the v2 `inventory.workspace` apply, see above.
...
### Phase 7c — equipped talismans writer (new write path)
...
- **Files (planned scope)**: extension of the equipment writer from Phase 7b, or a sibling writer; apply layer extension; tests; frontend preview row.
...
- **Risks**: medium — relies on Phase 7b infrastructure.
...
Phase 7a.2 / 7b+ work remains design-only in this document. Each phase requires a separate user approval before implementation per the workflow in `~/.claude/CLAUDE.md`.
