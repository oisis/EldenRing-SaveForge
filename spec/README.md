...
| 56 | [Templates v2 — Partially Implemented Extension](56-templates-v2.md) | `partial` | Additive extension of [55](55-build-template.md). Phase 0..6 + Phase 6b + Phase 7a + Phase 7a.2 + Phase 7b.0 + Phase 7b.1 + Phase 7c + Phase 9 shipped: schema v2 additive (`version: 2`, top-level `selection`, `sections.profile` + `sections.stats`), global Templates library shell, public YAML import/export (library on disk stays JSON), create-from-character flow with per-field profile/stats selection + preview + Save to Library, v2 metadata badge in library and preview, **Phase 5 = library Apply for profile/stats via `ApplyBuildTemplateV2FromLibraryToCharacter` and Phase 5D.2 direct imported-YAML Apply via `ApplyBuildTemplateV2ToCharacterJSON` on the canonical JSON produced by the import preview (both `mode: append`, `profile.class` skipped, snapshot + rollback), Phase 6 = apply-time overrides for the same profile/stats subset on both surfaces through frontend-only mutation of the canonical JSON forwarded to the same JSON endpoint, no backend / bindings / `App.tsx` change, Phase 6b = apply-time weapon level override for the existing v1 `inventory.workspace` Apply path through an additive `WeaponLevelOverride { Enabled bool, StandardLevel *int, SomberLevel *int }` runtime option on `app_templates.go::ApplyTemplateOptions`, applied after each template-side weapon patch via `editor.UpdateWeapon` + `editor.ClampUpgrade(req, MaxUpgrade)`, surfacing `weapon_level_clamped` / `weapon_unupgradeable` warnings on `report.Warnings`; UI controls live inside the existing `SortOrderTab.tsx` Templates dropdown, `TemplatesShellModal` / `ApplyOverridesPanel` / `ImportTemplatePreviewModal` / `TemplateLibraryModal` untouched; no v2 schema change, no equipment writer, no direct save mutation, Phase 7a = first real v2 apply path for `inventory.workspace` routed through the active `InventoryEditSession` / `InventoryWorkspaceSnapshot` via the new `App.GetActiveInventoryEditSessionForCharacter(charIdx) → { active, sessionID }` lookup and the additive `ApplyTemplateV2Options.SessionID`; missing session → `IssueCodeInventorySessionRequired`, unknown/wrong-character session → `IssueCodeInventorySessionInvalid`; mixed profile+stats+inventory.workspace apply is atomic via a dual slot+workspace snapshot rollback; the v2 inventory apply never touches `slot.Data` directly — `SaveInventoryWorkspaceChanges` remains the only commit point; `ImportTemplatePreviewModal` / `TemplateLibraryModal` lift their supported-section sets to `{profile, stats, inventory.workspace}`; `TemplatesShellModal` looks up the active session for every v2 apply surface (library, direct YAML import, URL import, `Apply with overrides…`), Phase 7a.2 = apply-time weapon level override threaded into the Phase 7a v2 `inventory.workspace` apply path through an additive `WeaponLevelOverride` field on `ApplyTemplateV2Options` (reusing the v1 type and `validateWeaponLevelOverride` validator verbatim, replacing the two hard-coded `nil` override pins at the v2 inventory + storage `applyTemplateItemsToWorkspace` call sites with `opts.WeaponLevelOverride`); runtime-only option (never inside the canonical JSON); new `WeaponLevelOverridePanel` embedded in the existing `ApplyOverridesModal` and rendered only when `selection.inventory.workspace` is present (testids `apply-overrides-weapon-{panel,enabled,standard,somber,error}`); fast library Apply (`ApplyBuildTemplateV2FromLibraryToCharacter`) still sends no override; structurally-valid override + profile/stats-only template is silently ignored (no items → no-op); `weapon_level_clamped` / `weapon_unupgradeable` warnings flow into `ApplyTemplateV2Result.Preview.Warnings`; v1 `SortOrderTab.tsx` Phase 6b dropdown untouched; no new v2 schema section, no equipment writer, no direct save mutation, no `App.tsx` change, Phase 9 = `https://` URL import through `PreviewBuildTemplateImportYAMLFromURL` under the full §12.3 SSRF guard list (HTTPS-only, pre-connect IP filter on literal and DNS-resolved addresses, redirect re-check ×3, 1 MiB body cap, 10 s total / 5 s idle timeouts, strict TLS, Content-Type allowlist, no auth/cookies/headers, struct-typed YAML decode reused unchanged; the URL preview reuses the same `ImportTemplatePreviewModal`, so Save to Library / Apply to character / Apply with overrides all ship unchanged on the URL surface; no library schema change — `sourceURL` metadata is not added)**. v2 apply for sections outside `{profile, stats, inventory.workspace}` remains blocked; `ApplyBuildTemplateV2FromFileToCharacter` exists backend/bindings-side but is intentionally unwired in UI because the JSON path is WYSIWYG with the preview. Phase 7b.0 (shipped 2026-05-31) = backend-only `(s *core.SaveSlot).WriteEquipment([]EquipmentWrite) error` foundation for ChrAsmEquipment slots 0–9 + 12–15 (weapons + ammo + armor); strict GaMap-present-only resolution, validate-then-mutate atomicity, targeted hash 7 / 8 recompute, no template schema change / App method / Wails binding / frontend. Phase 7b.1 (shipped 2026-06-01) = end-to-end v2 `sections.equipment` through the Phase 7b.0 writer — 14-slot `EquipmentSection` (weapons LH/RH 1/2/3, arrows1/2, bolts1/2, armor head/chest/arms/legs) with `EquipmentItemRef` carrying `BaseItemID` + optional `Name` / `Upgrade` / `InfusionName` / `AoWItemID` (BaseItemID 0 = explicit clear, upgrade nil = match any); app-layer scanner + resolver searches `slot.Inventory.CommonItems` only (storage not searched), emits `equipment_item_not_in_inventory` warnings + skip on miss and `equipment_item_ambiguous` + first-match-wins on duplicates; `WriteEquipment` dispatched inside the existing dual-snapshot rollback scope after profile/stats `SyncPlayerToData`; `equipment + inventory.workspace` combo hard-rejected (GaMap freshness) with `equipment_inventory_combo_unsupported`; equipment-only templates do NOT require a session and reuse the canonical JSON path on library / direct YAML / URL surfaces; `ImportTemplatePreviewModal` + `TemplateLibraryModal` expose equipment as the fourth supported section; no `App.tsx` / `SortOrderTab.tsx` / `ApplyOverridesPanel.tsx` change, no equipment override panel. Phase 7c (shipped 2026-06-01) = v2 talisman apply by extending `sections.equipment` with `talisman1..5` (ChrAsmEquipment indices 17–21, hash 8) instead of introducing a separate `sections.equippedTalismans` — talismans live in the same `ChrAsmEquipment` struct and reuse the Phase 7b.1 resolver, combo guard, preview row, frontend gate, and rollback; `backend/core/equipment_writer.go` adds `EquipSlotTalisman1..5`, a fourth class `slotClassTalisman` (only accepts the `ItemTypeAccessory` 0xA0 handle prefix), the hash 8 widening to 17–21, and `GaMap[handle]`-direct encoding (no `| 0x80000000` mask); resolver gates non-empty talisman refs against active pouch capacity `1 + effective profile.talismanSlots` (clamped to `MaxProfileTalismanSlots = 3`, cap = 4) with a new `talisman_slot_pouch_insufficient` warning + skip; Talisman5 with `baseItemID > 0` always warn + skip (vanilla cap = 4); `talisman5: {baseItemID: 0}` clear always allowed; mixed `profile.talismanSlots + equipment.talismanN` lifts the cap before equipment apply runs; `equipment + inventory.workspace` combo hard reject unchanged; no frontend source-code change (`V2_APPLY_SUPPORTED_SECTIONS` already includes `equipment`); `frontend/wailsjs/go/models.ts` shows no diff (talisman extensions are JSON-payload fields on `EquipmentSection`). Phase 7d.0 → 7d.4 (shipped 2026-06-02) = end-to-end spells import / preview / apply through a new `sections.spells` schema (14 named `Spell1..Spell14` pointer slots, prefix `0x40000000` shared by sorceries AND incantations, `BaseItemID == 0` = explicit clear, nil pointer = leave unchanged), `(s *core.SaveSlot).WriteSpells` batch writer with targeted hash[10] recompute (the global `RecalculateSlotHash` stays unwired in production; `WriteEquipment` still owns hash[7] / hash[8]), `db.ItemIDToMagicParamID(itemID) = itemID & 0x0FFFFFFF` 28-bit-mask helper, full frontend gating + preview row + library badge; spells coexist freely with all other shipped sections (no combo restrictions); the `WriteSpells` call sits **after** `vm.MapViewModelToSlot` AND **after** `slot.WriteEquipment` so the VM flush cannot clobber the new spells region. Phase 7d.4b (`BuildTemplateV2ExportOptions.EquippedSpellsRaw` wiring + `CreateTemplateV2Modal` Spells checkbox), Phase 8 (appearance via preset), Phase 10 (multi-character pack) still planned. |
